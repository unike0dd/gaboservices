const ROUTES = { health: '/health', debugConfig: '/debug/config', submit: '/careers' };
...
export default {
  async fetch(request, env) {
    ...
    if (method === 'GET' && path === ROUTES.health) {
      return jsonResponse({ ok: true, worker: 'careers-intake', routes: Object.values(ROUTES) }, 200, request, env);
    }
    if (method === 'GET' && path === ROUTES.debugConfig) {
      return jsonResponse({ ok: true, worker: 'careers-intake', hasAppsScriptUrl: Boolean(String(env.APPS_SCRIPT_BRIDGE_URL || '').trim()), hasBridgeToken: Boolean(String(env.APPS_SCRIPT_BRIDGE_TOKEN || '').trim()), hasSharedSecret: Boolean(String(env.SOLITARY_TO_CORREO_SHARED_SECRET || '').trim()), allowedOriginsCount: getAllowedOrigins(env).length }, 200, request, env);
    }
    if (method !== 'POST' || path !== ROUTES.submit) {
      return jsonResponse({ ok: false, stage: 'route', error: 'Not found.' }, 404, request, env);
    }
    return proxyUpstream(request, env, 'careers');
  }
};
...
async function proxyUpstream(request, env, routeKey) {
  ...
  const upstream = await fetch(appsScriptUrl, {
    method: 'POST',
    headers: { 'content-type': 'application/json; charset=utf-8', 'x-apps-script-bridge-token': bridgeToken, 'x-gabo-hop': 'careers-intake', 'x-solitary-route': routeKey },
