...
Completed in code (follow-up):
- **Step 1 (Server/client policy parity):** Added `enforced_by` ownership metadata to behavior controls in `chatbot/behavior.yml` and added automated validation via `scripts/validate-chatbot-policy-parity.js`.
- **Step 5 (Automated regression):** Added automated chatbot regression smoke checks (`scripts/chatbot-regression-smoke.js`) and enforced both parity + regression checks in CI workflow (`.github/workflows/security-compliance.yml`).

## Cloudflare Worker enforcement note

Because chatbot traffic is sent to a Cloudflare Worker endpoint, policy ownership for high-trust controls has been updated to `enforced_by: cloudflare` in `chatbot/behavior.yml` (integrity checks, rate limiting, output filtering, interaction gatekeeping, monitoring, and maintenance). This keeps policy metadata aligned with the actual enforcement plane.
